...
-    *alpha* :
 - Vérifie la validité de la syntaxe d'une chaine alphabétique. N'accepte que des caractères alphabétiques qui peuvent être accentués.
...
-    *alnum* :
 - Vérifie la validité de la syntaxe d'une chaine alphanumérique, accepte des mots et des chiffres ainsi que des caractères de ponctuation.
 - Utilisation : $objet->setRule('alnum', "message");
-    *num* :
 - Vérifie la validité de la syntaxe d'une chaine de chiffres, n'accepte que des chiffres.
...
-    *minlength* :
 - Vérifie que la chaine comporte au moins un certain nombre de caractères. On doit indiquer en troisième paramètre un entier correspondant au nombre minimum de caractères attendus.
 - Utilisation : $objet->setRule('minlength', "message", 10);
-    *maxlength* :
 - Vérifie que la chaine comporte au maximum un certain nombre de caractères. On doit indiquer en troisième paramètre un entier correspondant au nombre maximum de caractères attendus.
...
-    *differentDe* :
 - Vérifie que la valeur saisie est différente de l'argument. Par exemple, pour une liste de sélection dont le premier item affiche « Sélectionnez une valeur » et dont l'attribue value vaut « -1 », on indique cette valeur comme ne pouvant être sélectionnée. Ça équivaut à rendre un choix obligatoire dans une liste.
...
4.  pour certaines autres règles un seconde valeur de contrôle (*rangelength*, *rangeval*) : dans ce cas, le troisème paramètre sera un tableau indexé contenant les deux valeurs à utiliser comme éléments de contrôle.

###Le chainage des méthodes
Dès la création d'un objet pour un champ de formulaire donné, on peut chainer les méthodes et s'affranchir ainsi de la nécessité de ré-écrire la variable pour chaque appel d'une nouvelle méthode.
...
####NOTE : il n'est pas exclus que le code de la partie validation soit simplifié dans un avenir rapproché de façon à rendre la création de classe de validation supplémentaires moins compliqué même si en pratique ça ne présente actuellement que peu de difficultés.
## Intégrer le formulaire dans un template
